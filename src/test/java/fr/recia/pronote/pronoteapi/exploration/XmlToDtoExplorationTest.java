...
class XmlToDtoExplorationTest {
    ...
    void printJsonFromXml() throws IOException {
        String xml;
        try (InputStream in = getClass().getResourceAsStream("/mock/pronote-mock-eleve.xml")) {
            xml = new String(in.readAllBytes(), StandardCharsets.UTF_8);
        }

        when(fetchPronoteService.getPronoteXmlAsString()).thenReturn(xml);

        var service = new FetchAndParseEleveDataServiceFromEleveImpl(
